...
$success = "";
$error_message = "";
$conn = mysqli_connect("localhost","root","","finalyr");
if(!empty($_POST["submit_email"])) {
	$result = mysqli_query($conn,"SELECT * FROM borrowers WHERE email='" . $_POST["email"] . "'");
	...
	if($count>0) {
		...
		if($mail_status == 1) {
			$result = mysqli_query($conn,"INSERT INTO otpexpiry(otp,is_expired,create_at) VALUES ('" . $otp . "', 0, '" . date("Y-m-d H:i:s"). "')");
...
if(!empty($_POST["submit_otp"])) {
	$result = mysqli_query($conn,"SELECT * FROM otpexpiry WHERE otp='" . $_POST["otp"] . "' AND is_expired!=1 AND NOW() <= DATE_ADD(create_at, INTERVAL 24 HOUR)");
	$count  = mysqli_num_rows($result);
	if(!empty($count)) {
		$result = mysqli_query($conn,"UPDATE otpexpiry SET is_expired = 1 WHERE otp = '" . $_POST["otp"] . "'");
		$success = 2;	
	} else {
		$success =1;
